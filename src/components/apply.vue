  ...
  <div>
    ...
    <div>
      ...
      <span class="description">
        ...
        <h3>請輸入申請書內容</h3>
        <textarea v-model="message" style="border: none; height:100px; background-color: #2a2a2e;border-radius: 3vh;"
          placeholder="在此處輸入..." />
        <h3>開始時間 : </h3><input type="date" v-model="beginDate" class="datetime"><input type="time" v-model="beginTime"
          class="datetime">
        <h3>結束時間 : </h3><input type="date" v-model="endsDate" class="datetime"><input type="time" v-model="endsTime"
          class="datetime">
      </span>
      <h3>Preview : {{ message }} </h3>
      <h3>開始時間 : {{ beginDate }} {{ beginTime }} 結束時間 : {{ endsDate }} {{ endsTime }}</h3>
      <span class="submit" v-on:click="submit()"><span class="word"><md-icon>add</md-icon>
          <h3 style="display: inline;">送出</h3>
        </span></span>
    </div>
  </div>
</template>
